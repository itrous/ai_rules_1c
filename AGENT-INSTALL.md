...
- **Source** — local clone of `1c-rules` if the user pointed at one; otherwise `https://github.com/itrous/ai_rules_1c` (default).
...
1. **Resolve the source.** If only a URL was given, clone it locally (`git clone https://github.com/itrous/ai_rules_1c.git <cache-dir>/1c-rules`) or reuse an existing clone.
...
5. **Render the MCP config** from `content/mcp-servers.json` according to the adapter's `mcp.schema` (mcpServers JSON dictionary, OpenCode `mcp[id]` schema, or Codex TOML `[mcp_servers.<id>]`). The stack mixes **two transports**: `bsl-analyzer-workspace` and `bsl-analyzer-reference` are **stdio** servers (no `url`; launched via a `command` + `args`, rendered as `{ type: "local", command: [...], … }` where the schema supports it), while `v8std` and `1c-code-check-mcp` (1С:Напарник) are **http** servers (a `url`). Renderers must branch on `url` vs `command` per server, not assume HTTP for all. **OpenCode only:** normalize each server key to start with a letter before writing it under `mcp` — leading `1c`/`1C` → `onec` (so `1c-code-check-mcp` → `onec-code-check-mcp`), any other non-letter-leading id gets an `mcp-` prefix; the `bsl-analyzer-*` and `v8std` ids already start with a letter, so they pass through unchanged. OpenCode names MCP tools `<server-key>_<tool>` and providers like Moonshot/Kimi reject function names that do not start with a letter (`^[a-zA-Z_]…`), which otherwise breaks the whole request with *"function name is invalid, must start with a letter"*. Canonical ids in `content/mcp-servers.json` and the docs stay as-is; only the OpenCode-rendered key changes for the `1c`-leading id. The other adapters keep the verbatim id (Cursor/Claude prefix tool names with `mcp_`/`mcp__`, so a digit-leading key is already safe there). **Target file & merge:** write each tool's MCP into the path declared by `mcp.target` — for OpenCode this is `opencode.json` at the **project root** (NOT `.opencode/opencode.json`, which OpenCode never reads; `.opencode/` holds only agents/commands/modes/plugins/skills/tools/themes), and for Kilo Code it is `.kilo/kilo.json` under the top-level `mcp` key. Both are **shared** user configs (`mcp.merge: true`): deep-merge **only** the top-level `mcp` key and preserve every other key the user has. **OpenCode validates each MCP entry with a strict schema** — emit only `{ type: "remote", url, enabled }` for http servers (or `{ type: "local", command: [...], enabled }` for the stdio `bsl-analyzer-*` servers); any extra key such as `description` or `connection_id` makes OpenCode reject the whole config and the servers silently never load. The repo's `content/mcp-servers.json` is a **PATH-based, secret-free template** (`bsl-analyzer` resolved on `PATH`, run from the project root so `--source-dir .` works; no `EMBEDDING_*` and no `NAPARNIK_TOKEN`); the canonical machine-specific entry is generated by `bsl-analyzer mcp install --target <tool> --preset recommended --source-dir .`, and 1С:Напарник reads its `NAPARNIK_TOKEN` from the wrapper's own environment, not from this file. After writing it, **recommend that the user restart the AI client** — see *Recommend a restart after MCP changes* below.
...
iex (irm https://raw.githubusercontent.com/itrous/ai_rules_1c/main/install.ps1)
iex "$(irm https://raw.githubusercontent.com/itrous/ai_rules_1c/main/install.ps1) init"
```
...
git clone https://github.com/itrous/ai_rules_1c.git $tmp
