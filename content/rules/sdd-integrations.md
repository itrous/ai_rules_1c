...
- **quick-spec** — change touches **one** existing metadata object **plus**, optionally, 1-3 independent isolated additions (a new constant, a new data processor / settings form, a new independent information register with no module). No new documents / accumulation or accounting registers / roles / event subscriptions / scheduled jobs. No changes to existing transactional paths, RLS conditions, posting code, or public common-module signatures. Naming of new objects is the only architecturally novel decision.
  *Evidence minimum:* targeted attribute check via `graph` action `resolve` (name → node) or `metadata` action `object` (see check 2 below) **plus** one `search` action `search_code`/`find_code` over the project's БСП modules (in `src/cf`) if the spec relies on a БСП subsystem **plus** a check of the host agent's project memory only if the change keywords overlap with prior project work. `Context sources` block — one line.
...
1. **Project memory — the host agent's native memory layer.** There is no memory MCP in this stack (see `AGENTS.md → Project memory`); consult the host agent's project memory (e.g. Claude Code project memory / the `memory.md` + `AGENTS.md → Project memory` markdown fallback). Run when the change keywords overlap with anything already touched in the project: existing object names (`НачислениеЗарплаты`, `ПродажиТовары`), known subsystems (`Документооборот`, `ИнтернетПоддержка`), recurring error messages, prior architectural decisions on the same domain. For genuinely greenfield topics — a domain the project has never touched — the memory check is optional; a single short note in `Context sources` ("memory check skipped: greenfield topic") is enough. Catches existing project conventions, naming quirks, settled architectural choices.
2. **Metadata facts — prefer targeted queries over full passports.** Choose the narrowest method that closes the gap:
   - **Single attribute / tabular-section column existence and type** — `graph` action `resolve` on `Документ.<Name>.Реквизит.<Attr>` (one call, minimal output) → `node` for the resolved id. Use this for "does object X have attribute Y of type T?" — by far the most common case.
   - **List of attributes / tabular parts / dimensions / resources / forms** — `metadata` action `object` (returns attributes with types, tabular sections, dimensions, resources, forms) and `metadata` action `tree` for browsing. Deterministic, no LLM. (No JSON-template / Cypher query language exists in this stack — use the `object`/`tree` actions.)
   - **Structural passport across many facets** — `metadata` action `object` is the structural-passport equivalent; pair it with `graph` action `node` / `neighbors` when you also need dependencies. There is no `sections` filter — the `object` action already returns a compact structural passport.
   - **Fallback chain on empty / non-actionable results** — `search` action `search_code` (semantic) → `find_code` (lexical / in-index substring retry) → only then `Grep` (per `AGENTS.md → Tooling & Standards → A.4`).
   Do not invent attribute names from analogous documents or from memory.
3. **Platform APIs — `bsl-analyzer-reference` (`syntax_help`, `search`) and ITS (`bsl-analyzer-reference its_help`; **v8std** `v8std_search` → `v8std_get_page`; Напарник `its_help` → `fetch_its` for versioned material).** Every platform type, method, or behaviour the spec relies on (`HTTPСоединение`, `ЗащищённоеСоединениеOpenSSL`, `ЗаписатьJSON` / `ПрочитатьJSON`, `ДлительныеОперации`, async / `Ждать`, role permissions, etc.) — verify the exact name, signature, and version availability against the project's `CompatibilityMode` **when the spec is normative about that API**. For version-specific platform docs use Напарник `search_1c_documentation` / `onec_help`. Memory-written API signatures are not evidence. Skip for hrestomatic APIs whose shape is fixed across all supported versions and where the spec does not pin a specific signature.
4. **БСП / SSL — `search` over the project's БСП modules (they live in `src/cf`) + **v8std** for БСП-related standards.** When the spec mentions integrating with a БСП subsystem (`Администрирование`, `ИнтернетПоддержкаПользователей`, `ПолучениеФайловИзИнтернета`, `ЦифроваяПодпись`, `ДлительныеОперации`, `ОчередьЗаданий`, `БезопасноеХранилище`, `ЗащитаПерсональныхДанных`, …), use `search` action `search_code`/`find_code` (and `metadata` action `tree`/`object`) to confirm the subsystem actually exists in this project's БСП version, its real name in this configuration, and which public API to call; `bsl-analyzer-reference` for the platform API behind it. Verify the БСП hook (`ПриОпределенииПодсистемСКоторымиВозможнаИнтеграция`, `ПриДобавленииЭлементовФормы`, etc.) exists in this БСП version. **Required without exception when the change introduces storage of secrets, tokens, or API keys** (confirm `БезопасноеХранилище` shape) **or touches personal data** (confirm `ЗащитаПерсональныхДанных` hooks).
5. **Project source patterns — `search` action `search_code` (semantic) / `find_code` (lexical) / `graph resolve`.** When the spec proposes a new module, function, or pattern, check whether the project already has a similar one to align naming, signature, and placement. Skip when the new code has no analog in the project (genuinely first-of-its-kind).
...
- **TODO / "to be clarified" / "уточнить" for a fact one MCP call closes.** If you can answer it now via host-agent memory / `graph resolve` / `metadata object` / `bsl-analyzer-reference syntax_help` / `search find_code`, do it now. A TODO is allowed only for facts that genuinely depend on a human decision (business rule, naming preference, priority).
...
- **Defensive MCP calls without a concrete gap.** Calling `metadata object` "for completeness" when a single `graph resolve` would close the only open question — same defect as a missing call.
...
- **If the decision depends on a 1C fact that one MCP call could close — make the call, do not ask.** The user is not a substitute for `graph resolve` / `metadata object` / `search find_code` / host-agent memory.
...
- **Closing an item from `design.md → ## Open Questions` with a self-justifying paragraph instead of a `CONFUSION` block.** Open Questions are by definition unresolved at design time; the agent does not have authority to close them unilaterally. A 1-2 paragraph rationale that picks an option (even a "minimal and reversible" one) is a defect of the same severity as bypassing `diagnostics file`, and is doubly so when the picked option modifies typical (standard) configuration objects (typical roles, typical forms, typical modules, typical event subscriptions). The only legitimate closure path is a `CONFUSION` block per `AGENTS.md → 1. Think Before Coding`, followed by the user's explicit choice. Implementation of the dependent task block does not start until that choice arrives.

### Banned questions at apply time — hard list

These questions MUST NEVER be asked during `/opsx:apply`, regardless of whether the corresponding `.dev.env` field is empty. Asking any of them is an apply-phase defect of the same severity as skipping `diagnostics file` after a BSL edit. The fallback is documented and applied silently — no question, no pause, no `AskUserQuestion` round.
