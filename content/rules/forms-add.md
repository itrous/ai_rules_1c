...
- `tooling-playbooks.md → Form Analysis and Generation` — full ordered list of MCP calls (`metadata` action `form` to study layout → `metadata` action `tree` / `object` for the owning object → write/modify XML → validate via `diagnostics` + the `1c-metadata-manage` skill's XML/XSD tooling → compile via the `1c-metadata-manage` skill).
...
- **XSD validation is mandatory** after any XML edit. There is no dedicated `verify_xml` MCP tool in this stack — validate the form XML against its schema using the `1c-metadata-manage` skill's XML/XSD tooling, and run `diagnostics` to surface metadata findings. A form that parses in your editor is not a form that loads in Designer.
