...
### Gate 1 — Syntax (`diagnostics file`)

- Run `diagnostics` action `file` on every touched `.bsl` module. No exceptions. (`diagnostics catalog` first if you don't yet know which codes the analyzer reports.)
...
- **`graph` action `callers`** on every changed export procedure / function (who depends on it), and **`graph` action `callees`** / **`neighbors`** to find what your change in turn touches. Resolve the durable node id first via `graph resolve` if you only have a name.
- Use **`graph` action `neighbors`** with `dir` / `edge_kinds` / `provenance` filters when you need a broader impact picture than pure caller/callee edges.
- For metadata changes (new attribute, renamed object, removed attribute): **`graph` action `neighbors`** filtered on the data-reference edge kinds (`query_ref`, `data_binding`, `manager_access`, `contains`, …) to list every metadata reference that needs to be reviewed.
...
**Graceful degradation — when no impact-analysis tool is exposed.** If the `bsl-analyzer-workspace graph` actions (`callers` / `callees` / `neighbors`) are not available in the current session (the server is not running), do **not** silently skip the gate. Instead:

1. State the fact explicitly in the Delivery summary under **Risks** as a fixed line: *"Impact analysis not run — `bsl-analyzer-workspace graph` not exposed in this session; downstream callers and metadata references were not enumerated."*
...
- Validate every modified XML file. There is no dedicated `verify_xml` MCP tool in this stack — run `diagnostics` (action `file`, then `workspace` for a sweep) to surface metadata findings, and use the `1c-metadata-manage` skill's XML/XSD tooling (v8unpack / schema checks) for schema-level validation. Pass criterion: zero schema violations.
...
- **Skipping Gate 1** "because the edit was tiny" — `diagnostics file` is the cheapest gate; skipping it never saves time.
- **Running gates in the wrong order** — running `check_1c_code` before `diagnostics file` wastes the AI checker on syntax-broken code.
