...
- **Do not ask the user about facts an MCP call could close.** Names, attributes, tabular sections, БСП subsystem availability, platform-API signatures — resolve via host-agent memory / `graph resolve` / `metadata object` / a `search` over the project's БСП modules / `bsl-analyzer-reference syntax_help` before reaching for `AskUserQuestion`.
...
- Resolve facts the project already knows (host-agent memory) or that an MCP call can close (`graph resolve`, `metadata object`, `search find_code`, `bsl-analyzer-reference syntax_help`) **before** reaching for `AskUserQuestion`.
