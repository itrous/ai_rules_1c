...
- Use the applicable verification toolset as concrete success criteria. For BSL / metadata changes: `diagnostics file`, Напарник `check_1c_code` / `review_1c_code`, ITS standards lookup, impact analysis via `graph` (`neighbors` / `callers`). For Markdown / rules / documentation: verify referenced paths, links, structure, and internal consistency.
...
3. **Verify before writing BSL / metadata / specs — scoped hard gate.** Use the minimum evidence set from `tooling-playbooks`: quick-fix BSL edits use only the directly relevant code / syntax context; full-cycle BSL changes use existing project code (project `search` + `graph`), metadata, and platform / БСП / ITS docs when those sources affect correctness; metadata XML / form changes use schema, examples, and metadata validation; **OpenSpec spec authoring** uses project memory for prior project notes plus the MCP tools that confirm every 1C fact the spec references (`metadata` / `graph` / `search` for object shape, platform / БСП / ITS docs for API and standards) — see `content/rules/sdd-integrations.md`. In the final answer for any non-trivial BSL / metadata change **or non-trivial OpenSpec spec authoring (proposal / design / tasks / delta specs that reference real 1C objects, APIs, or БСП subsystems)**, list the context sources actually used and briefly state why any normally relevant source was skipped. Skipping a relevant source silently counts as a defect.
4. **No blind chaining.** Every MCP call must close a concrete context gap. Follow the fallback order from `mcp-1c-tools`; do not duplicate calls or continue the chain after you already have enough evidence. The project-index ladder on `bsl-analyzer-workspace` is `search` action `search_code` (semantic) → `search` action `find_code` (lexical / FTS — this *is* the in-index substring retry, there is no separate flag) → `graph` (`resolve` → `callers` / `callees` / `neighbors`) / `metadata`. If a `graph` query returns empty / non-actionable results, reformulate (switch `search_code`↔`find_code`, adjust `edge_kinds` / `dir`, raise the result limit) before continuing. Before using `Grep` / `rg` for 1C project-source search, first exhaust this project-index path and state why those attempts were insufficient.
5. **Validate changed 1C code.** After BSL / metadata edits: `diagnostics file` on the edited module → Напарник `check_1c_code` → `review_1c_code`, within the verification budget defined in section B below. For metadata XML use schema / XML validation and prefer `1c-metadata-manage` for non-trivial changes.
...
1. **Verification budget — 1 call per validator by default; up to 3 only if the previous run returned a substantive defect.** Applies separately to `diagnostics file`, Напарник `check_1c_code`, and Напарник `review_1c_code` when validating BSL / metadata changes. A **cycle** = one logical edit of one module; every new edit starts a new cycle. `diagnostics` is a file / workspace analyzer (actions `catalog` / `file` / `workspace`), not a raw-text checker — run `diagnostics file` on the edited module after the edit; re-run only after an actual change.
   ...
   - **No-change repeats are forbidden.** Do not re-run `diagnostics file` or a Напарник validator against an unchanged module.
   - **After the limit** — fix the substantive errors and move on; style warnings do not block delivery.
   - **Pure metadata-XML changes (no BSL touched)** — `check_1c_code` and `review_1c_code` are usually irrelevant; skip them unless the metadata change embeds BSL (object / manager module, form module, fill-check expression, predefined-item population). Use metadata-XML validation once instead. `diagnostics file` is still run on any BSL module touched, even indirectly (e.g. a form module regenerated by the metadata skill).
   - Markdown / rules / documentation edits use the docs-fix path checks instead.
2. **AI-based MCP tools are non-deterministic.** Напарник `ask_1c_ai`, `rewrite_1c_code`, `modify_1c_code` produce drafts, not authority. Re-validate output via `diagnostics file` + Напарник `check_1c_code` + `review_1c_code` before delivery (subject to the budget above).
...
2. **No-change repeats are forbidden.** Do not repeat the same tool request against the same unchanged state when the previous result is still available: same search, same MCP query, same validator input, or same file contents. A repeat is allowed when parameters change substantially (different query, different object, different depth, different `action`), state changed (file edit, generated output, new checkout, resumed session, user edited the file), or freshness matters before a destructive action / final verification. Do not re-run `check_1c_code` / `review_1c_code` or `diagnostics file` if the code has not changed since the last run.
3. **Tune each query to the tool's schema.** The bsl-analyzer tools (`search`, `graph`, `metadata`, `diagnostics`, `query`, `execute`) are **action-dispatched** — one tool name plus an `action` parameter, with the remaining parameters depending on the action. Defaults are usually suboptimal. Before a parameter-rich call, consult `mcp-1c-tools/docs/<server>.md` (the environment descriptor wins on conflict if exposed) and tune to the task: pick the right `action` (e.g. `search_code` semantic vs `find_code` lexical), and the action's modifiers (`edge_kinds`, `provenance`, `dir`, `detail` on `graph`; `catalog` / `file` / `workspace` on `diagnostics`; `validate` / `execute` on `query`). Narrow scope with the action's own filters. If the first call returns nothing, reformulate (broaden / narrow, switch `search_code`↔`find_code`, adjust `edge_kinds` / `dir`, raise the result limit) before falling back to another tool. **This rule is about call quality — it does not relax the obligation to make the call.**
4. **Prefer structural tools over manual grep.** `graph` (`resolve` → `node` / `callers` / `callees` / `neighbors`) and `metadata` (`object` / `tree` / `form`) for code and metadata navigation — before falling back to substring search.
5. **Tools are action-dispatched — read the live schema, do not guess parameter names.** The bsl-analyzer tools do not have a fixed per-operation parameter list; the authoritative action and parameter set is whatever the live tool schema exposes in the current session. Do not carry over comol parameter names or invent a "natural-sounding" alias. Start an unfamiliar tool with its discovery action and read the returned shape:
   - `search` → `status`; `graph` → `overview` / `schema`; `diagnostics` → `catalog`; `metadata` → `info` / `tree`; `query` → `validate` on a trivial fragment; `execute` → `check`.
   - From that response, take the exact action names, parameter names, and value formats (1C dotted qualified names like `Справочник.Контрагенты`, `Документ.РеализацияТоваровУслуг`, `РегистрНакопления.ТоварыНаСкладах`, `ОбщийМодуль.РаботаСКонтрагентамиКлиентСервер`; durable `graph_id`s returned by `search` / `graph resolve`).
   - If a schema validator rejects the call as `Missing required argument` or `Unexpected keyword argument`, re-read the live schema (and `docs/<server>.md`) before retrying — do not paraphrase the parameter and do not retry with another guessed alias.
...
Two layers — `memory.md` (strict long-term store at project root) and the **host agent's native memory layer** (the AI client's own project-memory / notes feature, e.g. Claude Code project memory). There is **no memory MCP server in this stack** — the original `1c-templates-mcp` `remember` / `recall` has no bsl-analyzer equivalent (see `MIGRATION-bsl-analyzer.md` gap #2). Every project-specific note must land in one of the two layers, otherwise it is lost between sessions.
...
- **Host native memory** — primary store for everything else worth keeping: user corrections during work, non-obvious project-specific facts, recurring errors and fixes, naming and quirks of individual configuration objects. Record proactively when the user corrects you or clarifies a non-obvious detail; consult it at the start of any non-trivial task with key terms (object name, subsystem, error message). Write notes in English, one self-contained fact per note, preserving original 1C identifiers and object / module names as-is. No secrets / PII.
- **Availability.** Use the host agent's native memory whenever the active client exposes one. If the client has **no** native memory layer, append even small particular-case corrections to `memory.md` under a separate `## Captured during work (no host memory available)` section (eligibility criteria are temporarily relaxed) and migrate them to the host memory once it is available.
- **Promote / demote.** A note kept in host memory that later proves to meet all four `memory.md` criteria — promote to `memory.md` and remove the original. The same fact must not live in both stores.
...
- **verification-checklist** — unified "done" gate for any non-trivial change: ordered hard gates (`diagnostics file` → Напарник `check_1c_code` → `review_1c_code` → impact analysis via `graph` → metadata XML validation) plus soft gates (debug reproduction, plan adherence, user-explicit code review). Load before declaring any non-trivial change finished. File: `content/rules/verification-checklist.md`.
